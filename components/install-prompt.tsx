...
export default function InstallPrompt({ autoOpen = true, showTrigger = false }: InstallPromptProps) {
  ...
  useEffect(() => {
    ...
    if (autoOpen && !dismissed && iosBrowser) {
      setOpen(true)
  ...
  return (
    <>
      {showTrigger && !isStandalone && (
        <Button type="button" variant="outline" className="w-full" onClick={() => setOpen(true)}>
          <Download className="mr-2 h-4 w-4" />
          {choiceHandled ? 'Install App Again' : 'Install App'}
        </Button>
      )}
